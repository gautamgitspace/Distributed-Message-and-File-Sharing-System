...
def parseRELAYED(output):
	...
	if match:
		return 'From IP address:%s, To IP address:%s, Message:%s' % (match.group(1), match.group(2), match.group(3))
	else:
		return "\033[91mFailed to parse: RELAYED message\033[0m"

def parseRECEIVED(output):
	match = re.match('msg from:(.*?)\\n\[msg\]:(.*?)\\n', output)

	if match:
		return 'From IP address:%s, Message:%s' % (match.group(1), match.group(2))
	else:
		return "\033[91mFailed to parse: RECEIVED message\033[0m"
